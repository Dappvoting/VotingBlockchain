<template>
    ...
    <div class="mx-auto desktop:w-[1400px] max-sm:w-full desktop:px-0">
      <div class="my-4 bg-white p-4">
        ...
        <div class="w-full grid grid-cols-2 text-center">
          <div
            :class="{
              'font-bold': activeTab === 'ContestantsCampaigns',
              'border-b-2 border-red-900 text-red-900 ': activeTab === 'ContestantsCampaigns',
              'border-b-2 border-b-2-black': activeTab === 'ResultsCampaign',
            }"
            class="text-lg py-2  cursor-pointer"
            @click="changeContent('ContestantsCampaigns')"
          >
            Candidates
          </div>
          <div
            :class="{
              'font-bold': activeTab === 'ResultsCampaign',
              'border-b-2 border-red-900 text-red-900 ': activeTab === 'ResultsCampaign',
              'border-b-2 border-b-2-black': activeTab === 'ContestantsCampaigns',
            }"
            class="text-lg py-2  cursor-pointer"
            @click="changeContent('ResultsCampaign')"
          >
            Results
          </div>
        </div>
        <div class="py-4">
          <ContestantsCampaigns
            v-if="activeTab === 'ContestantsCampaigns'"
          ></ContestantsCampaigns>
          <ResultsCampaign
            v-if="activeTab === 'ResultsCampaign'"
          ></ResultsCampaign>
        </div>
      </div>
    </div>
  ...
  <script>
  import ContestantsCampaigns from "./ContestantsCampaigns.vue";
  ...
  export default {
    ...
    data() {
      return {
        activeTab: "ContestantsCampaigns", // Mặc định tab đầu tiên được chọn
      };
    },
    computed: {
      activeTabComponent() {
        return this.activeTab === "ContestantsCampaigns"
          ? ContestantsCampaigns
          : ResultsCampaign;
      },
    },
